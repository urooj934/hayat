...
import { View,Text,Picker,TouchableOpacity,Dimensions,TextInput,StyleSheet,KeyboardAvoidingView, Alert} from "react-native";
...
const RequestFood=({navigation})=>{
    ...
    const handlePostReq=()=>{
        ...
        createReqFoodDocument(title,desc,category,phoneno,);}
        }
    const createReqFoodDocument =async (title,desc,category,phoneno,address)=>{
        const reqFood = await firestore.collection('ReqFood').doc();
            try{
              reqFood.set({
    ...
    <View style={styles.smallCircle} ></View>
        ...
        <Animatable.Image source={require('../assets/food.png')}
    animation="bounceIn"
    duration={1500}
