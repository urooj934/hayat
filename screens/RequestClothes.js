...
import { View,Text,TouchableOpacity,Dimensions,ImageBackground,TextInput,StyleSheet,KeyboardAvoidingView ,Alert } from "react-native";
import FontAwesome from 'react-native-vector-icons/FontAwesome'
import FontAwesome5 from 'react-native-vector-icons/FontAwesome5'
import * as Animatable from 'react-native-animatable';
import {auth, firestore } from "../firebase/firebase";
const RequestClothes=()=>{
    const[title,setTitle]=React.useState('');
    const[desc,setDesc]=React.useState('');
    const[category,setCategory]=React.useState('');
    const[phoneno,setPhoneno]=React.useState('') ;
    const handlePostReq=()=>{
        createReqObjectDocument(title,desc,category,phoneno,address);
        }
    const createReqObjectDocument =async (title,desc,category,phoneno,address)=>{
