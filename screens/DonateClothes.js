...
import { View,Alert,Text,TouchableOpacity,ImageBackground,Dimensions,TextInput,StyleSheet,ActivityIndicator, ScrollView,KeyboardAvoidingView } from "react-native";
...
const DonateFood=({navigation})=>{
    ...
    const handleobjectsdonation=()=>{
        if (title==''||desc==''||category==''||phoneno=='' ){
            ...
            createdonateObjectsDocument(title,desc,category,phoneno,location);
        }
        }
        const createdonateObjectsDocument =async (title,desc,category,phoneno,location)=>{
            const imageUrl = await uploadimg();
        console.log('Image Url: ', imageUrl);
            const donateobject = await firestore.collection('donateobject').doc();
            try{
              donateobject.set({
    ...
    <View style={styles.smallCircle} ></View>
     ...
     color:"#666666",fontWeight:'bold'}}>Donate clothes</Text> 
        <Animated.View style={{marginTop:10, opacity: Animated.add(0.3,Animated.multiply(fall,1.0)) }}>
        ...
        <ScrollView style={{marginBottom:30,padding:15,flex:1}}>  
  ...
  {uploading?(<View><Text>{transferred}% completed</Text>
  <ActivityIndicator size='large' color='#B100FF'/></View>):(  
    <TouchableOpacity style={styles.commandButton} onPress={handleobjectsdonation}>
